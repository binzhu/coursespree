function deleteNotification(el, id, event) {
	var parameters = {delete_notification:id, ajax:1, method: 'get'};
	if (event) {
		Object.extend(parameters, {event:1});
	}
	ajaxRequest(el, location.toString(), parameters, onDeleteNotification);	
}
...
function sendQueueMessages(el) {
	...
	var parameters = {ajax:1, method: 'get'};
	ajaxRequest(el, 'send_notifications.php', parameters, onSendQueueMessages);
}
...
function clearQueueMessages(el) {
	...
	var parameters = {delete_all_notifications:1, ajax:1, method: 'get'};
	ajaxRequest(el, location.toString(), parameters, onClearQueueMessages);	
}
...
function sendQueueMessage(el, id) {
	var parameters = {notification_id:id, ajax:1, method: 'get'};
	ajaxRequest(el, 'send_notifications.php', parameters, onSendQueueMessage);	
}
function onSendQueueMessage(el, response) {
	new Effect.Fade(el.up().up());
}
function clearQueueMessage(el, id) {
	var parameters = {delete_notification:id, ajax:1, method: 'get'};
	ajaxRequest(el, location.toString(), parameters, onClearQueueMessage);	
}
function onClearQueueMessage(el, response) {
	if (response.evalJSON(true).status) {
		new Effect.Fade(el.up().up());
	} else {
		alert('Some problem emerged');
...
var isEvent;
function activateNotification(el, notification_id, is_event) {
    Element.extend(el);
    ...
    if (row_element.className.match("deactivated")) {
        url = location.toString()+'&postAjaxRequest=1&activate_notification='+notification_id+'&event='+is_event;
        willBecomeActive = true;        
    } else {
        url = location.toString()+'&postAjaxRequest=1&deactivate_notification='+notification_id+'&event='+is_event;
