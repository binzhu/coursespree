 ...
 {else}
  ...
  <table class="statisticsGeneralInfo">
   <tr class="{cycle name = 'common_skill_info' values = 'oddRowColor, evenRowColor'}">
...
<!--ajax:skillUsersTable-->
  <table id="skillUsersTable" sortBy=0 size="{$T_TABLE_SIZE}" activeFilter="1" class="sortedTable" useAjax="1" url="{$smarty.server.PHP_SELF}?ctg=statistics&option=skill&sel_skill={$smarty.get.sel_skill}{$T_STATS_FILTERS_URL}&" style="width: 100%">
   <tr class="topTitle">
...
{if $T_CURRENT_SKILL_INFO}
 {eF_template_printBlock title = "`$smarty.const._STATISTICSFORSKILL` <span class='innerTableName'>&quot;`$T_CURRENT_SKILL_INFO->skill.description`&quot;</span>" data = $smarty.capture.skill_statistics image = '32x32/reports.png' help = 'Reports'}
{else}
 {eF_template_printBlock title ="`$smarty.const._SKILLSSTATISTICS`" data = $smarty.capture.skill_statistics image = '32x32/reports.png' help ='Reports'}
{/if}
