	...
	if(!(isError())) {
		...
		if(!(isError())) {
			$sql = "INSERT INTO `".DB_PREFIX."users` SET
						fName='$fName', lName='$lName', email='$email', userName='$userName', password='".$password."', active='$active', dated='$dated'
					";
			$res = mysql_query($sql);
			echo $sql;
...
if(isset($_POST['sell_step_2'])) {
	...
	if(!(isError())) {
		$sql = "UPDATE `".DB_PREFIX."notices` SET coverImage='$coverImage', professorID='$professor', docTypeID='$docType', docName='$docName', tips='$tips', price='$price' WHERE id='$docID'";
		$res = mysql_query($sql);
		if($res) {
...
if(isset($_POST['publish'])) {
	...
	if(!(isError())) {
		$sql = "UPDATE `".DB_PREFIX."notices` SET price='$price', status='1' WHERE id='$docID'";
		$res = mysql_query($sql);
		...
		if(!(isError())) {
			...
			if(!(isError())) {
				if($quesID > 0) {
					...
					$sql = "INSERT INTO `".DB_PREFIX."questions` SET userID='{$_SESSION['userID']}', caption='$caption', description='$description', catID='$catID', price='$price', tags='$tags', dated='$dated'";
