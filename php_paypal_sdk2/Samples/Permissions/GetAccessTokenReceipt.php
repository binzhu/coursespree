...
session_start();
		...
		try {
			$verifier=$_REQUEST['txtVerifier'];
			$request_array= array(
			GetAccessToken::$token => $_REQUEST['txtRequesttoken'],
			RequestEnvelope::$requestEnvelopeErrorLanguage=> 'en_US',
			);
			if(!empty($verifier))
			{
				$request_array[GetAccessToken::$verifier]=$verifier;
			}
	...
	$resArray=hash_call('Permissions/GetAccessToken',$nvpStr);
